---
title: 隐私协议
date: 2025-11-17 15:44:08
---

# 隐私协议
隐私协议最新更新时间为：2025-11-17

---

本站非常重视用户的隐私和个人信息保护。你在使用网站时，可能会收集和使用你的相关信息。通过《隐私政策》向你说明在你访问 https://www.yichencv.top/ 网站时，如何收集、使用、保存、共享和转让这些信息。

## 一、访问时如何收集和使用你的个人信息
### 在访问时，收集访问信息的服务器会收集以下信息（但不限于）：
- 网络身份标识信息（浏览器 UA、IP 地址）
- 设备信息
- 浏览过程操作方式、浏览方式与时长、性能与网络加载情况

### 在访问时，本站内置的第三方服务会通过以下或更多途径，来获取你的以下信息（但不限于）：
- 百度统计工具: 收集你的访问信息、访问操作过程
- 51la 统计工具: 收集你的访问信息
- 灵雀应用监控平台: 收集你的访问操作过程和资源加载情况
- busuanzi 统计: 收集你的访问信息
- CloudFare: 收集你的访问信息
- Vercel: 收集你的访问信息
- 阿里 cdn（iconfont）: 收集你的访问信息
- QQ 音乐: 收集你的访问信息
- 网易云音乐: 收集你的访问信息

### 在访问时，本人仅会出于以下目的，使用你的个人信息：
- 用于网站的优化与文章分类，用户优化文章
- 恶意访问识别，用于维护网站
- 恶意攻击排查，用于维护网站
- 网站点击情况监测，用于优化网站页面
...
### 你应该知道，在访问时，以下信息（但不限于）会被第三方获取并使用：
第三方部分为了抵抗攻击、使用不同节点 CDN 加速等需求会收集以下信息（但不限于）：
- 网络信息：IP地址、国家、省份、城市、运营商
- 设备信息：设备、浏览器信息
### (示例数据):
IP地址：192.168.11.138
国家：中国
...
城市：长沙
运营商：电信
设备：Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Safari/537.36 Edg/124.0.0.0
*(此页面如果未能获取到信息并不代表无法读取上述信息，以实际情况为准。)*

## 二、评论时如何收集和使用你的个人信息
评论使用的是无登录系统的匿名评论系统，你可以自愿填写真实的或虚假的信息作为你评论的展示信息。
鼓励你使用不易被人恶意识别的昵称进行评论，但是建议你填写真实的邮箱以便收到回复（邮箱信息不会被公开）。
在你评论时，会额外收集你的个人信息。
在评论时，本站内置的第三方服务会通过以下或更多途径，来获取你的相关信息：

- **cravatar** : 收集你的访问信息、评论填写的个人信息用于展示头像
在访问时，本人仅会出于以下目的，收集并使用以下信息：
...
关于如何在 Chrome 中清除、启用和管理 Cookie，请参考 **Chrome 帮助文档**。
